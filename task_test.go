...
func TestOptionsMarshal(t *testing.T) {
	...
	if tu.Options.Offset != nil && *tu.Options.Offset != time.Hour {
...
func TestOptionsEdit(t *testing.T) {
	...
	t.Run("delete deletable option", func(t *testing.T) {
		tu := &platform.TaskUpdate{}
		tu.Options.Offset = ptrutil.DurationPtr(0)
		newscript := `option task = {cron: "* * * * *", name: "foo"}
...
from(bucket: "x")
	|> range(start: -1h)`
		...
		if !cmp.Equal(*tu.Flux, newscript) {
			t.Fatalf(cmp.Diff(*tu.Flux, newscript))
		}
	})
