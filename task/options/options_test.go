...
import (
	...
	"github.com/influxdata/influxdb/internal/ptrutil"
...
func TestFromScript(t *testing.T) {
	for _, c := range []struct {
		...
		{script: scriptGenerator(options.Options{Name: "name0", Cron: "* * * * *", Concurrency: ptrutil.Int64Ptr(2), Retry: ptrutil.Int64Ptr(3), Offset: ptrutil.DurationPtr(-time.Minute)}, ""), exp: options.Options{Name: "name0", Cron: "* * * * *", Concurrency: ptrutil.Int64Ptr(2), Retry: ptrutil.Int64Ptr(3), Offset: ptrutil.DurationPtr(-time.Minute)}},
		{script: scriptGenerator(options.Options{Name: "name1", Every: 5 * time.Second}, ""), exp: options.Options{Name: "name1", Every: 5 * time.Second, Concurrency: ptrutil.Int64Ptr(1), Retry: ptrutil.Int64Ptr(1)}},
		{script: scriptGenerator(options.Options{Name: "name2", Cron: "* * * * *"}, ""), exp: options.Options{Name: "name2", Cron: "* * * * *", Concurrency: ptrutil.Int64Ptr(1), Retry: ptrutil.Int64Ptr(1)}},
		{script: scriptGenerator(options.Options{Name: "name3", Every: time.Hour, Cron: "* * * * *"}, ""), shouldErr: true},
		{script: scriptGenerator(options.Options{Name: "name4", Concurrency: ptrutil.Int64Ptr(1000), Every: time.Hour}, ""), shouldErr: true},
		{script: "option task = {\n  name: \"name5\",\n  concurrency: 0,\n  every: 1m0s,\n\n}\n\nfrom(bucket: \"test\")\n    |> range(start:-1h)", shouldErr: true},
		{script: "option task = {\n  name: \"name6\",\n  concurrency: 1,\n  every: 1,\n\n}\n\nfrom(bucket: \"test\")\n    |> range(start:-1h)", shouldErr: true},
		{script: scriptGenerator(options.Options{Name: "name7", Retry: ptrutil.Int64Ptr(20), Every: time.Hour}, ""), shouldErr: true},
		{script: "option task = {\n  name: \"name8\",\n  retry: 0,\n  every: 1m0s,\n\n}\n\nfrom(bucket: \"test\")\n    |> range(start:-1h)", shouldErr: true},
...
func TestValidate(t *testing.T) {
	good := options.Options{Name: "x", Cron: "* * * * *", Concurrency: ptrutil.Int64Ptr(1), Retry: ptrutil.Int64Ptr(1)}
	...
	*bad = good
	bad.Offset = ptrutil.DurationPtr(1500 * time.Millisecond)
	if err := bad.Validate(); err == nil {
		t.Error("expected error for sub-second delay resolution")
	}

	*bad = good
	bad.Concurrency = ptrutil.Int64Ptr(0)
	if err := bad.Validate(); err == nil {
		t.Error("expected error for 0 concurrency")
	}

	*bad = good
	bad.Concurrency = ptrutil.Int64Ptr(math.MaxInt64)
	if err := bad.Validate(); err == nil {
		t.Error("expected error for concurrency too large")
	}

	*bad = good
	bad.Retry = ptrutil.Int64Ptr(0)
	if err := bad.Validate(); err == nil {
		t.Error("expected error for 0 retry")
	}

	*bad = good
	bad.Retry = ptrutil.Int64Ptr(math.MaxInt64)
	if err := bad.Validate(); err == nil {
